...
class DatabaseManager:
    ...
    def save_incident(selfself, text: str, risk_level: int, justification: str, media_type: str) ->Optional[str]:
        try:
    ...
    def save_video-metadata(self, bucket_name: str, bucket_id: str, duration: float, file_size: int) -> Optional[str]:
    try:
        with self.conn.cursor() as cur:
            cur.execute(
                """
    ...
    def save_audio_metadata(self, bucket_name: str, bucket_id: str, duration: float, file_size: int) -> Optional[str]:
        """Similar ao save_video_metadata, mas para áudios"""

    def get_incident_with_media(self, incident_uid: str) -> dict:
